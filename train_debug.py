...
for epoch in range(EPOCHS):
    ...
    for batch_idx, (wm_imgs, clean_imgs) in enumerate(train_loader):
        ...
        # ---- SHAPE DEBUG (solo ep=1, batch=0) ----
        if ep == 1 and batch_idx == 0:
            print("\n" + "="*60)
            print(">>> SHAPE DEBUG — Ep 1, Batch 0")
            print(f"  Input UNet   (wm_imgs):        {tuple(wm_imgs.shape)}")
            print(f"  Target       (clean_imgs):     {tuple(clean_imgs.shape)}")
            print(f"  Output UNet  (reconstructed):  {tuple(reconstructed_imgs.shape)}")
            print(f"  Logits UNet  (bit-logit):      {tuple(logits_reconstructed.shape)}")
            with torch.no_grad():
                out_dbg = detector.detect(clean_imgs)
                preds_dbg = out_dbg["preds"]
                print(f"  detector.detect(clean_imgs)['preds']:      {tuple(preds_dbg.shape)}")
                print(f"    -> c0 detection score:                   {tuple(preds_dbg[:,0:1].shape)}")
                print(f"    -> c1: bit-logit ref:                    {tuple(preds_dbg[:,1:].shape)}")
                out_dbg2 = detector.detect(reconstructed_imgs)
                preds_dbg2 = out_dbg2["preds"]
                print(f"  detector.detect(reconstructed)['preds']:   {tuple(preds_dbg2.shape)}")
            print(f"  Range wm_imgs:       [{wm_imgs.min():.3f}, {wm_imgs.max():.3f}]")
            print(f"  Range reconstructed: [{reconstructed_imgs.min():.3f}, {reconstructed_imgs.max():.3f}]")
            print("="*60 + "\n")
        # ------------------------------------------
